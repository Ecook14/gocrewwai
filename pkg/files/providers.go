...
func ValidateFile(file File, provider string) error {
	...
	switch ft {
	case TypeImage:
		if !constraints.SupportsImage {
			return fmt.Errorf("provider %s does not support images", provider)
		}
	case TypePDF:
		if !constraints.SupportsPDF {
			return fmt.Errorf("provider %s does not support PDFs", provider)
		}
	case TypeAudio:
		if !constraints.SupportsAudio {
			return fmt.Errorf("provider %s does not support audio", provider)
		}
	case TypeVideo:
		if !constraints.SupportsVideo {
			return fmt.Errorf("provider %s does not support video", provider)
		}
	}
